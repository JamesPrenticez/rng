...
export class GameEngine {
  ...
  gameLoop = () => {
    ...
    // Draw the player sprite (THIS WAS MISSING!)
    this.player.drawPlayerSprite();
  ...
  private drawCameraInfo(): void {
    ...
    // Camera position
    ctx.fillText(
      `Camera: (${Math.round(this.camera.position.x)}, ${Math.round(
    ...
    ctx.fillText(
      `Player: (${Math.round(this.player.position.x)}, ${Math.round(
    ...
    ctx.fillText(
      `Player Screen: (${Math.round(playerScreenPos.x)}, ${Math.round(
        playerScreenPos.y
      )})`,
      10,
      60
